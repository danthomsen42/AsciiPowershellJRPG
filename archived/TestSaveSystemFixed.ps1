...
if (Test-Path $SaveDirectory) {
    Write-Host "OK Save directory exists: $SaveDirectory" -ForegroundColor Green
} else {
...
if (Test-Path $AutoSaveFilePath) {
    try {
        ...
        Write-Host "OK Auto-save file exists and is valid JSON" -ForegroundColor Green
        ...
        Write-Host "ERROR Auto-save file is corrupted: $($_.Exception.Message)" -ForegroundColor Red
    }
} else {
    Write-Host "INFO Auto-save file not found (expected on first run)" -ForegroundColor Yellow
}
...
for ($i = 1; $i -le 5; $i++) {
    ...
    if (Test-Path $SlotFilePath) {
        try {
            ...
            Write-Host "OK Slot $i`: Level $($slotData.Player.Level) - $($slotData.GameInfo.SaveTime)" -ForegroundColor Green
        } catch {
            Write-Host "ERROR Slot $i`: Corrupted save file" -ForegroundColor Red
        }
    } else {
        Write-Host "INFO Slot $i`: [Empty]" -ForegroundColor DarkGray
    }
}
...
if (Test-Path $QuickSaveFilePath) {
    try {
        ...
        Write-Host "OK Quick Save: Level $($quickData.Player.Level) - $($quickData.GameInfo.SaveTime)" -ForegroundColor Cyan
    } catch {
        Write-Host "ERROR Quick Save: Corrupted save file" -ForegroundColor Red
    }
} else {
    Write-Host "INFO Quick Save: [Empty]" -ForegroundColor DarkGray
}
...
if (Test-Path $LegacySavePath) {
    try {
        ...
        Write-Host "OK Legacy save file found" -ForegroundColor Green
        ...
        Write-Host "ERROR Legacy save file is corrupted" -ForegroundColor Red
    }
} else {
    Write-Host "INFO No legacy save file found" -ForegroundColor Yellow
}
