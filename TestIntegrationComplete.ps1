...
. ".\Display.ps1" -ErrorAction SilentlyContinue
...
if ($global:Player -and $global:Party) {
    Write-Host "   ✓ Conditional loading will work - custom party detected"
    Write-Host "   ✓ Display.ps1 will NOT overwrite Player.ps1"
    Write-Host "   ✓ Display.ps1 will NOT overwrite default party"
} else {
    Write-Host "   ✗ Conditional loading will fail - missing variables" -ForegroundColor Red
}
...
if ($defaultParty.Count -eq 4 -and $global:PartyTrail.Count -eq 8 -and $global:Player -and $global:Party -and $positions.Keys.Count -eq 4) {
    Write-Host "`n   🎉 INTEGRATION TEST PASSED! Character creation is ready!" -ForegroundColor Green
} else {
    Write-Host "`n   ❌ INTEGRATION TEST FAILED! Issues found." -ForegroundColor Red
}
